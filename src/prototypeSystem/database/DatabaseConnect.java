...
//            conn.prepareStatement("insert into TextRPGSave(name,level,maxHealth,maxEXP,currentEXP,strengthAbility,dexterityAbility) values ('b', 1,100,100,100,100,100);").execute();
        ...
        while (resultSet.next()) {
            ...
            player = new Player(id, name, level, maxHealth, currentExp, currentExp, strengthAbility, dexterityAbility, intelligenceAbility, usedExp, mapUnlock ,money);
    ...
    public void savePlayer(Player player) throws SQLException {
        String sql = "update TextRPGSave Set name = ?,level = ?,maxHealth = ?,currentEXP = ?,strengthAbility = ?,dexterityAbility = ?,intelligenceAbility = ?,usedExp = ?, mapUnlock =?, money =? where id=?";
        ...
        psmt.setInt(11, player.getId());  //얘가 무조건 끝에 와야함
        psmt.executeUpdate();
